...
11. **RECONSTRUCTION OF ALL VISUALS (CRITICAL):**
    *   **ALL UI Elements and Content-like Visuals (Recreate):** Any non-textual visual elements that are part of the **UI design itself OR appear as content within the design** (e.g., icons, logos, photographs, complex background patterns, decorative shapes, QR-codes) **MUST be FLAWLESSLY and PIXEL-PERFECTLY RECONSTRUCTED using ONLY HTML and CSS, or INLINE SVG within the HTML.** This demands advanced CSS (e.g., complex gradients, \`clip-path\`, \`mask-image\`, pseudo-elements, filters) and potentially meticulous SVG path data. **YOU CANNOT USE <img> TAGS TO EMBED RASTERIZED VERSIONS OF THESE UI ELEMENTS FROM THE ORIGINAL IMAGE OR THE SOURCE \`photoDataUri\`**. For icons, prefer recreating with CSS/SVG or using inline SVG representations (e.g., similar to Lucide Icons). If an image-like element cannot be reasonably recreated with CSS/SVG (e.g., a very complex photograph integral to the design), you must use a solid color block or a CSS gradient that visually approximates the original's dominant colors and feel in that area.
