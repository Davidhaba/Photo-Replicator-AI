...
1.  **Output Format:** The output MUST be a single string containing a complete HTML document, including \`<html>\`, \`<head>\`, and \`<body>\` tags. **CRITICAL: Ensure the entire document is generated and nothing is omitted or truncated. If you anticipate the full HTML/CSS for perfect replication will be extremely long and might exceed response limits, you MUST prioritize generating a COMPLETE, non-truncated HTML document that captures the overall structure and essential visual elements, even if it means simplifying some of the absolute finest details to fit. An incomplete, truncated document is unacceptable.**
...
3.  **Uncompromising Meticulous Detail Replication (within completeness constraints):** You must achieve **pixel-perfect accuracy** where possible. Pay **obsessive, forensic-level attention** to the precise positioning (x, y coordinates), dimensions (width, height, to the exact pixel), colors (extract or infer exact hex/RGB/HSL values), font styles (if identifiable, use the exact font; otherwise, find the closest web-safe match that replicates the visual character, weight, size, letter spacing, line height), spacing (margins, padding), borders (thickness, style, color, radius), shadows (offset, blur, color, spread), gradients (type, direction, color stops), and **every single visual attribute** present in the image. **Absolutely NO detail is too small to be ignored, simplified, or approximated UNLESS it's to ensure the document's completeness as per instruction #1. If a visual effect exists in the image, it MUST be replicated in the HTML/CSS.**
...
5.  **Structural and Visual Integrity:** Recreate the structural layout, color palette, and ALL key visual elements from the image with the **highest possible fidelity**. Imagine you are creating a perfect digital forgery of the image using only HTML and CSS. The output must be a **pixel-for-pixel representation** wherever achievable with HTML/CSS. **Do not simplify, omit, or approximate any visual element unless absolutely impossible to render with HTML/CSS OR to ensure document completeness (see instruction #1), in which case, use CSS to create the closest possible visual effect (e.g., complex gradients, abstract shapes).**
...
7.  **Valid and Clean Code:** Ensure the HTML is well-formed and valid. While visual accuracy and completeness are paramount, strive for reasonably clean and efficient HTML/CSS.
